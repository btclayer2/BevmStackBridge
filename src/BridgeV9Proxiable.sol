...
import "./lib/SystemWithdrawV3.sol";
import "./lib/Types.sol";

// Bridge V9: 20251219
contract BridgeV9Proxiable is Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ////////////////////////////////
    ...
    struct BridgeV6Storage {
        ...
        uint256 _badBridgeV8ProxiableStorage1; // 32 bytes (BridgeV6StorageLocation + 8)
        uint256 _badBridgeV8ProxiableStorage2; // 32 bytes (BridgeV6StorageLocation + 9)
    ...
    function reinitialize(string memory /*_network*/ ) public reinitializer(6) {
        _clearBridgeV8ProxiableStorage();
    }
    ...
    function _clearBridgeV8ProxiableStorage() internal {
        BridgeV6Storage storage $ = _getBridgeV6Storage();

        $._badBridgeV8ProxiableStorage1 = 0;
        $._badBridgeV8ProxiableStorage2 = 0;
    }

    /**
     * @notice Estimates the rent for a given account
     * @notice tx.origin is address(0) or tx.origin have enough balance for rent
     * @dev This is a view function that queries the estimated rent without modifying state
     * @param account The address of the account for which rent is being estimated
     * @return uint256 The estimated rent amount
     * @return uint64 The estimated rent duration (days)
     */
    function estimateRent(address account) public view returns (uint256, uint64) {
        return SystemWithdraw.estimateRent(account);
    }

    /**
     * @notice This function retrieves the current rent settings from the SystemWithdraw contract.
     * @return uint256 The first return value, likely representing the daily rent balance.
     * @return uint64 The second return value, likely representing the active timestamp(ms).
     */
    function rentSettings() public view returns (uint256, uint64) {
        return SystemWithdraw.rentSettings();
    }
}
