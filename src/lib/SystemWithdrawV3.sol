...
import "./IWithdrawPrecompileV2.sol";
...
library SystemWithdraw {
    ...
    function estimateRent(address account) internal view returns (uint256, uint64) {
        (bool success, bytes memory returnData) =
            precompile().staticcall(abi.encodePacked(IWithdrawPrecompile.estimateRent.selector, account));
        require(success, string(returnData));

        require(returnData.length == 64, "Invalid returnData");
        (uint256 rentBalance, uint64 rentDays) = abi.decode(returnData, (uint256, uint64));

        return (rentBalance, rentDays);
    }

    function rentSettings() internal view returns (uint256, uint64) {
        (bool success, bytes memory returnData) =
            precompile().staticcall(abi.encodePacked(IWithdrawPrecompile.rentSettings.selector));
        require(success, string(returnData));

        require(returnData.length == 64, "Invalid returnData");
        (uint256 dailyRent, uint64 startTime) = abi.decode(returnData, (uint256, uint64));

        return (dailyRent, startTime);
    }
}
